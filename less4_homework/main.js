...
function arrReceiver(arr) {
    ...
    for (let i = 0; i < array.length; i++) {
        console.log('arr number:', array[i])
    }
}
...
function paragraphCreator(paragraph) {
    document.write(`<div>`)
    document.write(`<h2>${paragraph}</h2>`)
...
function exchange(sumUAH,currencyValues,exchangeCurrency){
 let finnalySum = sumUAH / exchangeCurrency
if (currencyValues === 'USD'){
    console.log('exchange UAH to USD:',finnalySum)
}else if (currencyValues === 'EUR'){
    console.log('exchange UAH to EUR:',finnalySum)
}else {
    console.log('currency is not accepted')
}
}
exchange(10000,'USD',42)
